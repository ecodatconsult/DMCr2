...
uploadFinal <- function(user, psw){
  ...
  daten <- c("daten","standorte","ereignis")

  for(data in daten){

      if(data == "daten"){

        daten_DB_import <- dbGetQuery(con,"SELECT * FROM import.bilder")
        dbWriteTable(con, c("fotofallen", "bilder"),
                     daten_DB_import, row.names = FALSE, overwrite = TRUE)
      }

      if(data == "standorte"){

        standorte_DB_import <- pgGetGeom(con, query = "SELECT * FROM import.fotofallen_standorte;")
        pgInsert(con, c("fotofallen","fotofallen_standorte"), standorte_DB_import, overwrite = TRUE)
      }

      if(data=="ereignis"){

        ereignisse_DB_import <- dbGetQuery(con,"SELECT * FROM import.ereignisse")
        dbWriteTable(con, c("fotofallen", "ereignisse"),
                     ereignisse_DB_import, row.names = FALSE, overwrite = TRUE)
        ...
        dbSendQuery(con,
                    "update
                  fotofallen.fotofallen_standorte
                  set
                  ts_startbild = ereignisse.ts
                  from
                  fotofallen.ereignisse
                  where
        ...
        dbSendQuery(con,
                    "update fotofallen.ereignisse set fluchtreaktion = 0
                  where tierart_1 in ('Rothirsch','Reh') and fluchtreaktion is null;")

        dbSendQuery(con,
                    "UPDATE fotofallen.ereignisse SET kamera_id = lower(kamera_id);")
        ...
        dbSendQuery(con,
                    "update fotofallen.fotofallen_standorte
                set laufzeit =
        ...
        #  "update fotofallen.fotofallen_standorte set raster_id = fotofallen_raster.raster_id from fotofallen.fotofallen_raster
        ...
        #        "alter table fotofallen.fotofallen_standorte add column \"n_", tolower(i),"\" integer;", sep = "")))
        #    dbSendQuery(con, stri_enc_toutf8(paste(
        #      "alter table fotofallen.fotofallen_standorte add column \"n_", tolower(i),"_woche\" double precision;", sep = "")))}

        # for (i in tierartenliste_altersklassen) {
        #   for (j in altersklassen) {
        #     dbSendQuery(con, stri_enc_toutf8(paste(
        #       "alter table fotofallen.fotofallen_standorte add column \"n_", tolower(i),"_", tolower(j),"\" integer;", sep = "")))
        #   dbSendQuery(con, stri_enc_toutf8(paste(
        #     "alter table fotofallen.fotofallen_standorte add column \"n_", tolower(i),"_", tolower(j),"_woche\" double precision;", sep = "")))}}

        #
        # dbSendQuery(con, stri_enc_toutf8(
        #   "alter table fotofallen.fotofallen_standorte add column n_rothirsch_2u3w integer; -- 2w + 3w"))
        # dbSendQuery(con, stri_enc_toutf8(
        #     "alter table fotofallen.fotofallen_standorte add column n_rothirsch_2u3w_woche double precision;"))
        # dbSendQuery(con, stri_enc_toutf8(
        #   "alter table fotofallen.fotofallen_standorte add column n_rothirsch_2u3m integer; -- 2m + 21m + 22m + 3m"))
        # dbSendQuery(con, stri_enc_toutf8(
        #   "alter table fotofallen.fotofallen_standorte add column n_rothirsch_2u3m_woche double precision;"))
        # dbSendQuery(con, stri_enc_toutf8(
        #   "alter table fotofallen.fotofallen_standorte add column n_rothirsch_0 integer;"))
        # dbSendQuery(con, stri_enc_toutf8(
        #   "alter table fotofallen.fotofallen_standorte add column n_rothirsch_0_woche double precision;"))
        ...
        where \"n_",tolower(i),"\" is NULL and
          ...
          #from fotofallen.fotofallen_standorte where verwertbar = 1
          #group by raster_id)
          #update
          #fotofallen.fotofallen_raster
          #set
